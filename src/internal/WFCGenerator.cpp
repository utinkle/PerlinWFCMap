...
private:
    ...
    TileMap generateWithManualRules(const HeightMap& heightmap,
                                   const TileMap& terrainMap,
                                   uint32_t width, uint32_t height,
                                   const WFCParams& params) {
        ...
        // 初始化WFC单元格
        std::vector<WFCCell> cells(width * height);
        std::set<uint32_t> allPatterns;
        for (uint32_t i = 0; i < m_patterns.size(); i++) {
            allPatterns.insert(i);
        }
        ...
        for (uint32_t y = 0; y < height; y++) {
            for (uint32_t x = 0; x < width; x++) {
                ...
                // 根据地形过滤可能的模式
                std::set<uint32_t> possible;
                for (uint32_t patternIdx : allPatterns) {
                    const Pattern& pattern = m_patterns[patternIdx];
                    TerrainType center = pattern.tiles[pattern.width * pattern.height / 2];
                    
                    // 检查中心是否与地形兼容
                    auto it = m_terrainAdjacencyRules.find(terrain);
                    if (it != m_terrainAdjacencyRules.end()) {
                        if (it->second.find(center) != it->second.end()) {
                            possible.insert(patternIdx);
                        }
                    }
                }
                
                cells[idx].possiblePatterns = possible;
                cells[idx].entropy = cells[idx].calculateEntropy();
            }
        }
        ...
        for (uint32_t iteration = 0; iteration < params.iterations; iteration++) {
            // 选择最小熵的单元格
            int32_t minEntropyCell = -1;
            float minEntropy = std::numeric_limits<float>::max();
            
            for (uint32_t i = 0; i < cells.size(); i++) {
                if (!cells[i].collapsed && cells[i].entropy < minEntropy) {
                    minEntropy = cells[i].entropy;
                    minEntropyCell = i;
                }
            }
            
            if (minEntropyCell == -1) break; // 所有单元格都已坍缩
            
            // 坍缩单元格
            collapseCell(cells[minEntropyCell], params.temperature);
            
            // 传播约束
            propagateConstraints(cells, width, height, minEntropyCell, params);
            
            // 更新熵
            updateEntropies(cells);
        }
        
        // 生成最终地图
        for (uint32_t i = 0; i < cells.size(); i++) {
            if (cells[i].collapsed) {
                decorationMap[i] = (uint32_t)cells[i].collapsedType;
            } else if (!cells[i].possiblePatterns.empty()) {
                // 选择第一个可能的模式
                uint32_t patternIdx = *cells[i].possiblePatterns.begin();
                const Pattern& pattern = m_patterns[patternIdx];
                decorationMap[i] = (uint32_t)pattern.tiles[0];
            }
        }
    ...
    TileMap generateWithLearnedPatterns(const HeightMap& heightmap,
                                       const TileMap& terrainMap,
                                       uint32_t width, uint32_t height,
                                       const WFCParams& params) {
        // 简化的WFC实现
    ...
    void learnPatternsFromExample(const TileMap& example,
                                 uint32_t exampleWidth, uint32_t exampleHeight,
                                 uint32_t patternSize) {
        m_patterns.clear();
        m_patternHashes.clear();
        
        // 滑动窗口提取模式
        for (uint32_t y = 0; y <= exampleHeight - patternSize; y++) {
            for (uint32_t x = 0; x <= exampleWidth - patternSize; x++) {
                std::vector<TerrainType> pattern;
                
                // 提取patternSize x patternSize的模式
                for (uint32_t dy = 0; dy < patternSize; dy++) {
                    for (uint32_t dx = 0; dx < patternSize; dx++) {
                        uint32_t idx = (y + dy) * exampleWidth + (x + dx);
                        pattern.push_back(static_cast<TerrainType>(example[idx]));
                    }
                }
                
                // 计算模式哈希
                uint32_t hash = hashPattern(pattern);
                
                if (m_patternHashes.find(hash) == m_patternHashes.end()) {
                    m_patterns.emplace_back(pattern, patternSize, patternSize, 1.0f);
                    m_patternHashes[hash] = m_patterns.size() - 1;
                }
            }
        }
        
        // 建立邻接规则
        buildPatternAdjacencyRulesFromExample(example, exampleWidth, exampleHeight, patternSize);
    }
    
    TileMap generateWithLearnedPatternsFromScratch(uint32_t width, uint32_t height,
                                                  const WFCParams& params) {
        // 基于学习到的模式生成新地图
        TileMap output(width * height, 0);
        ...
        // 简化的WFC生成
        std::vector<std::set<uint32_t>> possibilities(width * height);
        ...
        for (auto& possibility : possibilities) {
            possibility = allPatterns;
        }
        
        // WFC迭代
        for (uint32_t iter = 0; iter < params.iterations; iter++) {
            // 选择要坍缩的单元格
            std::vector<uint32_t> uncollapsed;
            for (uint32_t i = 0; i < possibilities.size(); i++) {
                if (possibilities[i].size() > 1) {
                    uncollapsed.push_back(i);
                }
            }
            
            if (uncollapsed.empty()) break;
            
            std::uniform_int_distribution<> dist(0, uncollapsed.size() - 1);
            uint32_t cellIdx = uncollapsed[dist(m_rng)];
            
            // 坍缩单元格
            if (!possibilities[cellIdx].empty()) {
                std::vector<uint32_t> possible(possibilities[cellIdx].begin(), 
                                               possibilities[cellIdx].end());
                std::uniform_int_distribution<> patternDist(0, possible.size() - 1);
                uint32_t chosenPattern = possible[patternDist(m_rng)];
                ...
                // 放置模式
                for (uint32_t dy = 0; dy < pattern.height && y + dy < height; dy++) {
                    for (uint32_t dx = 0; dx < pattern.width && x + dx < width; dx++) {
                        uint32_t idx = (y + dy) * width + (x + dx);
                        output[idx] = (uint32_t)pattern.tiles[dy * pattern.width + dx];
                        
                        // 限制邻居的可能性
                        restrictNeighborPossibilities(idx, chosenPattern, possibilities, 
                                                      width, height, params);
                    }
                }
                
                possibilities[cellIdx].clear();
                possibilities[cellIdx].insert(chosenPattern);
            }
        }
        
        // 填充未决定的单元格
        for (uint32_t i = 0; i < output.size(); i++) {
            if (output[i] == 0 && !possibilities[i].empty()) {
                uint32_t patternIdx = *possibilities[i].begin();
                const Pattern& pattern = m_patterns[patternIdx];
                output[i] = (uint32_t)pattern.tiles[0];
            }
        }
                ...
                if (nx >= 0 && nx < static_cast<int>(width) &&
                    ...
                    if (newPossible != oldPossible) {
                        ...
                        if (!newPossible.empty()) {
                            ...
                            // 回溯或失败
                            if (params.enableBacktracking) {
                                // 简化：重置邻居
                                cells[neighborIdx].possiblePatterns = oldPossible;
                                cells[neighborIdx].entropy = cells[neighborIdx].calculateEntropy();
                            }
                        }
                    }
    ...
    bool patternsCompatible(uint32_t patternA, uint32_t patternB, int dx, int dy) {
        const Pattern& a = m_patterns[patternA];
        const Pattern& b = m_patterns[patternB];
...
TileMap WFCGenerator::generateDecorationMap(const HeightMap& heightmap,
                                           const TileMap& terrainMap,
                                           uint32_t width, uint32_t height,
                                           const WFCParams& params) {
    return m_impl->generateDecorationMap(heightmap, terrainMap, width, height, params);
}

TileMap WFCGenerator::generateResourceMap(const TileMap& terrainMap,
                                         const TileMap& decorationMap,
                                         uint32_t width, uint32_t height,
                                         const WFCParams& params) {
    return m_impl->generateResourceMap(terrainMap, decorationMap, width, height, params);
}

TileMap WFCGenerator::generateFromExample(const TileMap& example,
                                         uint32_t exampleWidth, uint32_t exampleHeight,
                                         uint32_t outputWidth, uint32_t outputHeight,
                                         const WFCParams& params) {
    return m_impl->generateFromExample(example, exampleWidth, exampleHeight, 
                                      outputWidth, outputHeight, params);
}
